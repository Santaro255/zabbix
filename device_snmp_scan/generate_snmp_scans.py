...
data = """from pysnmp.hlapi import getCmd, SnmpEngine, CommunityData, UdpTransportTarget, ContextData, ObjectType, ObjectIdentity
import requests
import json
import socket
...
for i in net_list:  
    file = open('scan_devices_' + i + ".py", 'a')
    file.write(data.format(REPLACE=i))
    file.close()
